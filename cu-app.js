(() => {
  ...
  function ensureBrand(header, name) {
    ...
    if (brandWrap) {
      Array.from(brandWrap.querySelectorAll('img, svg'))
        .filter(node => !(toggleBtn && toggleBtn.contains(node)))
      ...
      if (!brandNameEl || brandNameEl === navTag) {
        brandNameEl = Array.from(info.querySelectorAll('a, span, strong, h1'))
          .find(el => el !== navTag && !el.classList.contains('cu-navtag'));
      }
      if (!brandNameEl || brandNameEl === navTag) {
        brandNameEl = document.createElement('span');
        info.insertBefore(brandNameEl, navTag.nextSibling);
      }
